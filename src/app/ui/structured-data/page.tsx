...
export default function StructuredDataPage() {
  ...
  // Extract handles on structured data interface
  const { submit,
    object, // recipe response using the schema
    isLoading, error, stop } = useObject({
              ...
              <ol className="space-y-4">
                {object.recipe.steps.map((step, index) => {
                  ...
                  const stepText = typeof step === 'string'
                    ? step
                    : (typeof step === 'object' && step !== null && 'step' in step)
                      ? String((step as { step: unknown }).step)
                      : String(step);
